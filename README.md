...
When `UPGRADE_SGLANG=1`, SGLang is cloned and `SGLANG_GIT_REF` is explicitly
fetched before its `python` package is installed. This supports ordinary branch
or commit names as well as GitHub pull-request refs such as
`pull/36497/head`, which a normal clone does not fetch automatically.
...
as `tokenizers` resolve from PyPI. Pin `SGLANG_GIT_REF` to the tested commit SHA
for a reproducible deployment after confirming Flash-Next works.
...
installs SGLang from [upstream PR #36497][5], which adds the model support that
is absent from the current `dev-cu13` image and published release wheel.
...
[5]: https://github.com/sgl-project/sglang/pull/36497 "Add Qwen3.8-Flash-Next model support — SGLang PR #36497"
