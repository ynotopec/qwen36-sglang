...
exec docker run "${DOCKER_RUN_CLEANUP_ARGS[@]}" "${DOCKER_TTY_ARGS[@]}" \
  ...
  -e ENABLE_DFLASH="${ENABLE_DFLASH:-0}" \
  -e DFLASH_DRAFT_MODEL_PATH="${DFLASH_DRAFT_MODEL_PATH:-}" \
  ...
  -e SPECULATIVE_DFLASH_DRAFT_WINDOW_SIZE="${SPECULATIVE_DFLASH_DRAFT_WINDOW_SIZE:-}" \
  "${DOCKER_OPTIONAL_ENV_ARGS[@]}" \
  -e SGLANG_ENABLE_SPEC_V2="${SGLANG_ENABLE_SPEC_V2}" \
  -e SGLANG_ENABLE_DFLASH_SPEC_V2="${SGLANG_ENABLE_DFLASH_SPEC_V2:-0}" \
  -e SGLANG_ENABLE_OVERLAP_PLAN_STREAM="${SGLANG_ENABLE_OVERLAP_PLAN_STREAM:-0}" \
